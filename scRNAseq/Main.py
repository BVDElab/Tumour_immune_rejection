...
mapdic = {0: "Macrophages_1", 1: "Macrophages_2", 2: "Macrophages_3", 3: "Macrophages_4",
          4: "Macrophages_5", 5: "Tumour cells", 6: "CD8+ T cells", 7: "Mfap5+ fibroblasts",
          8: "Erythrocytes", 9: "CD4+ T cells", 10: "Neutrophils", 11: "Tumour cells",
          12: "Dendritic cells", 13: "Mfap5- fibroblasts",
          14: "CD8+ T cells", 15: "Endothelial cells"}
...
s_genes = [x.lower().capitalize() for x in s_genes]
...
g2m_genes = [x.lower().capitalize() for x in g2m_genes]
...
mycmap = {"G1": sns.color_palette("husl")[1], "G2M": sns.color_palette("husl")[2], "S": sns.color_palette("husl")[3]}

ccycle = adata.obs.copy()
for agroup in set(ccycle["Pheno_name"]):
    groupframe = ccycle.loc[ccycle["Pheno_name"] == agroup].copy()
    fig, ax = plt.subplots(1, 2)
    i = 0
    for treatment in ["untreated", "treated"]:
        subframe = groupframe.loc[groupframe["treatment"] == treatment]
        thecounts = Counter(subframe["phase"])
        xvals = []
        yvals = []
        for x in thecounts.keys():
            yvals.append(x)
            xvals.append(thecounts[x])
        ax[i].pie(xvals, labels=yvals, autopct='%.1f%%', startangle=90, colors=[mycmap[x] for x in yvals])
        ax[i].axis('equal')  # Equal aspect ratio ensures that pie is drawn as a circle.
        i = i + 1
    ax[0].title.set_text('Untreated')
    ax[1].title.set_text('Treated')
    plt.tight_layout()
    plt.savefig("CellCycle" + agroup + "_pie.svg")
    plt.close()
